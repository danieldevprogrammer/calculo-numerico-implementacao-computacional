...
def gaussSeidel(A, b, tolerancia=1e-6, maxIteracoes=100):
    ...
    # Loop principal do método de Gauss-Seidel
    while erro > tolerancia and numIteracao < maxIteracoes:
        x_anterior = x.copy()  # Armazena as estimativas anteriores
        for i in range(n):
            soma1 = np.dot(A[i, :i], x[:i])
            soma2 = np.dot(A[i, i+1:], x_anterior[i+1:])
            x[i] = (b[i] - soma1 - soma2) / A[i, i]

        # Calcula o erro entre as estimativas atuais e as anteriores
    ...
    if numIteracao == maxIteracoes:
        print(Fore.RED + '\nO método de Gauss-Seidel atingiu o numéro máximo de iterações e não me retornou uma solução satisfatória.' + Style.RESET_ALL)
    ...
    # Exibe o resultado final que é a tabela, o número de iterações, o erro e o resíduo.
    ...
    # Exibe o resíduo por distância relativa
